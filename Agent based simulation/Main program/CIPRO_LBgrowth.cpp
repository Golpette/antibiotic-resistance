...
// --------------------------------------------------------------------------------------------------------
//  Bartek's code with following modifications:
//
//    - Growth rate is the LB-growth experimental data from Bartek
//    - Strains, fitnesses and MICs from Marcusson2009 are implemented
//    - Both daughter cells mutated
//    - Mutation rate depends on ciprofloxacin concentration
//    - Channels are now rectuangular, dimensions were determined from methyl-blue experiments
//          and diffusion code (see diffusion.cpp)
//    - ODs measured as 1mm^2 columns down centre of well 
//    - Driver mutations have individual probabilities (i.e. marR and acrR are 10 times gyrA1/2/parC)
//
// --------------------------------------------------------------------------------------------------------


// TODO:   ##DONE## - Use LB growth data in this program 
//         ##DONE## - Use correct OD measurement (cuboid down centre of well)
//         ##DONE## - Test effect of ciprofloxacin-dependent motility
//         ##DONE## -  Look at probabilities of mutational pathways & PM occurrences / well 
//         ##DONE## - Construct evolutionary pathway 
//         - rewrite code: genotype + mutation 
...
int wellpops[24];                       // no. of cells in each well 
 ...
 // size of the well [mm]. Square hole at the top y=(-hole/2,hole/2),z=(-hole,0)
const float w=3.25,  h=11.0,   hole_width=1.5,   hole_depth=3.75 ;   
...
const float gamma=1e-3 ;                  // mutation probability per daughter cell. 1e-2 is slightly higher than the base mutation rate (about 2e-3) in the absence of Cipro
...
const float max_growth_rate = 1.0 ;      // growth rate 1/h - 2 is max growth rate for E. coli (this is no incorporated in experimental data)
const float tmax=2*24;                   // duration of experiment in hours

const float init_bact = 100 ;           // initial number of cells in the first well
const float K = 1e4 ;                   // carrying capacity. Have removed "food".
//const float food0 = 1e6 ;             // NO LONGER USED, remove this from code


const float uni_antibiotic=0, max_antibiotic=1000;   // Cipro concentrations in ng/ml

const float MIC_WT = 32 ;                  // MIC of wild type
...
const float p_g0 = 1.0/1e3 ;     //gyrA1  - this is set to match expected mutations in experiment ( expect 0.1 gyrA1 mutations on avg per well)
...
// Mutation rate 'mu' depends on Cipro concentration, 'a', with form:  mu = gamma * [ 1 + ( X.a/MIC_WT )^Y  ], with some upper limit.
const float X = 15.0;   const float Y = 1.5;    //parameters governing form of mut-rate dependence on antibiotic.
const int max_increase = 200;                   //i.e. max_increase*gamma is max possible mutation rate per replication
...
//  LB (non-mixed) growth data   (length of arrays = 132 hardcoded into growth_function() method)
...
float Genotype::growth(int well) 
{
  ...
  // Check for all 28+3 types listed in Marcusson2009:
...
float growth_function( int wl )
{
  ...
  // where N is #cells in well and K is carrying capacity of well
  float g=-9.0 ;
...
// Print out genomic sequences in given well
...
//   EDITED FOR LB-GROWTH DATA 
    br=genotypes[cells[n].gen]->growth(cells[n].well) * growth_function( cells[n].well ) ;                 
    ...
    if (mode==0) { // reproduction (make 2 new cells, kill mother cell)
      if (_drand48()>genotypes[cells[n].gen]->death_during_rep(cells[n].well)) { // replication
        ...
        // Determine number of mutations in each daughter cell (modified to increase mutation probability with [ciprofloacin])
        float antib = well[  cells[n].well  ].a ;
        float mu = gamma * ( 1 + pow( X*antib/MIC_WT, Y)  ) ;
        if( mu > gamma*max_increase ) { mu = gamma*max_increase ; }
        ...
        //well[c.well].food -= 1.0 ; // Use 1 food per bacteria (no longer use food explicitly, but N/K dependence in Bartek's experimental growth data)
...
// now we implement death
    ...
    // print time and population to terminal
//    if (it%1000000==0) cout <<tt<<": "<<ntot<<"   "<<ntot_2<<endl ;

//  Data to produce Fisher plots -----------------------------------------
...
//  cout <<"reset\n"  ;
  ...
  for (int i=0;i<nwells;i++) {
    ...
    well[i].a=uni_antibiotic+pow(max_antibiotic,1.*(i+1)/nwells) ;  // So that well 0 has zero antibiotics
...
void init()
{
  ...
  sprintf(txt,"mkdir %s/sequenceFiles",NUM) ; system(txt) ;
  //
...
int main(int argc, char *argv[])
{
  ...
  // check mutation probabilites are less than 1
...
// DISABLED since for large K file size is huge ========================
...
// save all trajectories for cells present in each well if wellpop > 10% of carrying capaity
    for (int w=0;  w<nwells;   w++) {  
      if( wellpops[w] > K/10.0 ){
        sprintf(name,"%s/sequenceFiles/sequences_(well%d)_%d_%d.dat",NUM,w,RAND,sample) ;  
        save_sequences(name,w) ;   
      }
    }
/**
// Print reduced genotypes in "final well" REACHED IN EXPERIMENT
    bool findingFinalWell = true ; 
    while ( findingFinalWell ){
      for( int qq=nwells-1; qq>=0; qq-- ){
        if( wellpops[qq] > K/10.0 ){
          // Print genotype sequences in this well 
          sprintf(name,"%s/sequenceFiles/sequences_(well%d)_%d_%d.dat",NUM,qq,RAND,sample) ; 
          save_sequences(name,qq) ;
          findingFinalWell = false ;
          qq = -3 ; 
        }
      }
    }
