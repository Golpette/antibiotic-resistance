...
// PROOF OF PRINCIPLE.
// MC optimization of diffusion parameters in each well. 
...
//          carrying capacity was able to reproduce (i.e. infinite food). 1-food in getGrowthRate().




// Possible improvements:
//    (1) if number events expected is large, approximate Poisson distribution with normal.



public class TauLeap_MCopt_diff_FOOD{


    // ################## PARAMETERS ####################

    static final int num_optimisations = 10;
    ...
    static double[] D = new double[nwells] ;
    ...
    public static double getDist_timePoint(double[] simulation, Double[] experiment){
        //method to compare 2 sets of well densities at a given points in time  TODO:NOT IDEAL, requires time to match to 3 decimal places
        double dist = 0;
    ...
    public static void main(String args[])throws IOException{
        ...
        double[] D_hold = new double[nwells];
        double maxD = 0;   double minD = 0.2;
        for( int aw=0; aw<nwells; aw++){
            //if( aw%2==0){  D[aw] = 0.06 ; }
            //else{ D[aw] = 0.02 ; }
            //D_hold[aw]=0.04;
            D_hold[aw]=0.1*Math.random();
        }

        //double[] D_hold = {0.02707326615209555, 0.011409152166890814, 0.008918029175182376, 0.0035456564277820323, 0.01219900592230256, 0.01942387309752263, 0.024895653446228357, 0.019797099108474697, 0.02055910660830731, 0.07482389314737187, 0.028505054129130628, 0.03691570151499357, 0.008827552684524852, 0.03802116574578433, 0.0526220835417314, 0.05741334566246302, 0.03718992265326278, 5.265843985000038E-4, 0.04915705218770909, 0.005943121874366854, 0.005657668841789292, 0.005808587044792339, 0.0599478338005782, 0.019536838124053404};
	...
	for( int exp=0; exp<num_optimisations; exp++ ){
        ...
        for(int www=0; www<nwells; www++){
            double random_shift = (D[www] * 1.0) * (Math.random() - 0.5) ;   // PROB: WE WILL ONLY SHIFT SMALL VALUES BY TINY AMOUNT WITH THIS
...
// NOW USE FOOD DENSITY INSTEAD OF POPULATION DENSITY
		    ...
		    for( String key : wells[w].abundances.keySet() ){
	    		...
	    		wells[w].migration.put( key, num_of_strain * D[w] * timestep ) ;    
